...
import com.benet.fireflow.engine.condition.ConditionConstant;
...
public class SynchronizerInstance extends AbstractNodeInstance implements
    ...
    public transient static final String Extension_Target_Name = "com.benet.fireflow.kernel.SynchronizerInstance";
     ...
     */
    public void fire(ITokenInstance tk) throws KernelException {
        ...
        if (joinPoint.getAlive()) {
            ITokenInstance tokenForLoop = new TokenInstance(); // 产生新的token
        ...
        if (!doLoop) {//如果没有循环，则执行transitionInstance
                ...
                for (int i = 0; leavingTransitionInstances != null && i < leavingTransitionInstances.size(); i++) {
                    ...
                    TokenInstance token = new TokenInstance(); // 产生新的token
