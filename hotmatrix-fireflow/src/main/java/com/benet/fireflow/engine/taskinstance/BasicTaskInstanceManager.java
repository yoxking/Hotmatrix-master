...
import com.benet.fireflow.engine.calendar.ICalendarService;
...
import com.benet.fireflow.model.net.EndNode;
import com.benet.fireflow.model.net.Node;
import com.benet.fireflow.model.net.Synchronizer;
import com.benet.fireflow.model.net.Transition;
...
public class BasicTaskInstanceManager implements ITaskInstanceManager {
	 ...
	 * (org.fireflow.kenel.IActivityInstance)
	 */
	public final void createTaskInstances(ITokenInstance token,
		...
		for (int i = 0; i < activity.getTasks().size(); i++) {
			...
			IWorkTaskInstance taskInstance = this.createTaskInstance(
			...
			((WorkTaskInstance) taskInstance).setTaskType(taskType);
			((WorkTaskInstance) taskInstance).setStepNumber(token.getStepNumber());
			// ((TaskInstance) taskInstance).setTokenId(token.getId());
			((WorkTaskInstance) taskInstance).setProcessInstanceId(processInstance
					.getId());
			((WorkTaskInstance) taskInstance).setProcessId(processInstance
					.getProcessId());
			((WorkTaskInstance) taskInstance).setVersion(processInstance
					.getVersion());
			((WorkTaskInstance) taskInstance).setActivityId(activity.getId());
			if (Task.FORM.equals(taskType)) {
				((WorkTaskInstance) taskInstance)
						.setAssignmentStrategy(((FormTask) task)
								.getAssignmentStrategy());
				((WorkTaskInstance) taskInstance).setCanBeWithdrawn(true);
			} else {
				((WorkTaskInstance) taskInstance).setCanBeWithdrawn(false);
			}
			((WorkTaskInstance) taskInstance).setCreatedTime(calService
					.getSysDate());
			((WorkTaskInstance) taskInstance).setDisplayName(task.getDisplayName());
			((WorkTaskInstance) taskInstance).setName(task.getName());

			((WorkTaskInstance) taskInstance).setState(WorkTaskInstance.INITIALIZED);

			((WorkTaskInstance) taskInstance).setTaskId(task.getId());

			((WorkTaskInstance) taskInstance).setFromActivityId(token
					.getFromActivityId());
			...
			if (duration != null && calService != null) {
				((WorkTaskInstance) taskInstance).setExpiredTime(calService
						.dateAfter(calService.getSysDate(), duration));
			}

			// 2、保存实例taskInstance
			persistenceService.saveOrUpdateWorkTaskInstance(taskInstance);
	...
	public final void startTaskInstance(IWorkflowSession currentSession,
			IProcessInstance processInstance, IWorkTaskInstance taskInstance)
		...
		((WorkTaskInstance) taskInstance).setState(IWorkTaskInstance.RUNNING);
		((WorkTaskInstance) taskInstance).setStartedTime(this.rtCtx
				.getCalendarService().getSysDate());
		this.rtCtx.getPersistenceService().saveOrUpdateWorkTaskInstance(
				taskInstance);
	 ...
	 * @throws EngineException
	 */
	protected boolean activityInstanceCanBeCompleted(IWorkTaskInstance taskInstance)
			throws EngineException {
		IPersistenceService persistenceService = this.rtCtx
				.getPersistenceService();
		...
		if (thisActivity.getTasks().size() > 1) {
			List<IWorkTaskInstance> taskInstanceList = persistenceService
	...
	public final void completeTaskInstance(IWorkflowSession currentSession,
		...
		if (taskInstance.getState() == IWorkTaskInstance.COMPLETED
				|| taskInstance.getState() == IWorkTaskInstance.CANCELED) {
			return;
		}
		if (taskInstance.getState() == IWorkTaskInstance.INITIALIZED) {
			WorkflowProcess process = taskInstance.getWorkflowProcess();
			throw new EngineException(taskInstance.getProcessInstanceId(),
					process, taskInstance.getTaskId(),
		...
		((WorkTaskInstance) taskInstance).setState(IWorkTaskInstance.COMPLETED);
		((WorkTaskInstance) taskInstance).setCanBeWithdrawn(Boolean.FALSE);
		((WorkTaskInstance) taskInstance).setEndTime(rtCtx.getCalendarService()
				.getSysDate());
		persistenceService.saveOrUpdateWorkTaskInstance(taskInstance);
		// 触发相应的事件
		TaskInstanceEvent e = new TaskInstanceEvent();
		...
		List<ITokenInstance> tokens = persistenceService.findTokensForProcessInstance(
				taskInstance.getProcessInstanceId(), taskInstance
						.getActivityId());
		...
		ITokenInstance token = tokens.get(0);
	...
	public final void abortTaskInstance(IWorkflowSession currentSession,
		...
		persistenceService.abortTaskInstance((WorkTaskInstance) taskInstance);
	...
	public final void abortTaskInstanceEx(IWorkflowSession currentSession,
			IProcessInstance processInstance, IWorkTaskInstance thisTaskInst,
			String targetActivityId) throws EngineException, KernelException {
		// 如果TaskInstance处于结束状态，则直接返回
		if (thisTaskInst.getState() == IWorkTaskInstance.COMPLETED
				|| thisTaskInst.getState() == IWorkTaskInstance.CANCELED) {
			return;
		}
		...
		List<ITokenInstance> allTokens = null;
		...
		if (targetActivityId != null) {			
			...
			for (int i=0;allTokens!=null && i<allTokens.size();i++){
				ITokenInstance tokenTmp = allTokens.get(i);
...
//			}
		...
		// 第一步，首先Abort当前taskInstance
		persistenceService.abortTaskInstance((WorkTaskInstance) thisTaskInst);
		...
		if (targetActivityId != null) {
			...
			for (int i=0;i<allSynchronizersAndEnds.size();i++){
				...
				ITokenInstance tokenTmp =  new TokenInstance();
				...
				if (reachable){
					...
					ITokenInstance virtualToken = getJoinInfo(allTokens,synchronizer.getId());
					...
					if (tokenTmp.getValue()!=0){
						...
						persistenceService.saveOrUpdateTokenInstance(tokenTmp);
					}
				}
			}
		}
	 ...
	 * @throws EngineException
	 */
	protected final void fireTaskInstanceEvent(IWorkTaskInstance taskInstance,
	 ...
	 */
	public final WorkItemInstance createWorkItem(IWorkflowSession currentSession,
												 IProcessInstance processInstance, IWorkTaskInstance taskInstance,
		...
		wi.setState(IWorkItemInstance.INITIALIZED);
	 ...
	 */
	public final IWorkItemInstance claimWorkItem(String workItemId,
												 String taskInstanceId) throws EngineException, KernelException {
		IPersistenceService persistenceService = rtCtx.getPersistenceService();

		persistenceService.lockTaskInstance(taskInstanceId);

		IWorkItemInstance workItem = persistenceService.findWorkItemById(workItemId);

		if (workItem == null)
			return null;

		if (workItem.getState().intValue() != IWorkItemInstance.INITIALIZED) {
			WorkTaskInstance thisTaskInst = (WorkTaskInstance) workItem
					.getTaskInstance();
			throw new EngineException(thisTaskInst.getProcessInstanceId(),
					thisTaskInst.getWorkflowProcess(),
					thisTaskInst.getTaskId(),
		...
		if (workItem.getTaskInstance().getState().intValue() != IWorkTaskInstance.INITIALIZED
				&& workItem.getTaskInstance().getState().intValue() != IWorkTaskInstance.RUNNING) {
			WorkTaskInstance thisTaskInst = (WorkTaskInstance) workItem
					.getTaskInstance();
			throw new EngineException(thisTaskInst.getProcessInstanceId(),
					thisTaskInst.getWorkflowProcess(),
		...
		// 0、首先修改workitem的状态
		((WorkItemInstance) workItem).setState(IWorkItemInstance.RUNNING);
		((WorkItemInstance) workItem).setClaimedTime(rtCtx.getCalendarService()
				.getSysDate());
		...
		WorkTaskInstance taskInstance = (WorkTaskInstance) workItem.getTaskInstance();
	...
	public void completeWorkItem(IWorkItemInstance workItem,
			...
			throws EngineException, KernelException {
		if (workItem.getState().intValue() != IWorkItemInstance.RUNNING) {
			WorkTaskInstance thisTaskInst = (WorkTaskInstance) workItem
					.getTaskInstance();
		...
		((WorkItemInstance) workItem).setComments(comments);
		((WorkItemInstance) workItem).setState(IWorkItemInstance.COMPLETED);
		((WorkItemInstance) workItem).setEndTime(rtCtx.getCalendarService()
				.getSysDate());
		persistenceService.saveOrUpdateWorkItemInstance(workItem);
		...
		e.setWorkflowSession(((IWorkflowSessionAware) workItem)
				.getCurrentWorkflowSession());
		e.setProcessInstance(((WorkTaskInstance) workItem.getTaskInstance())
		...
		((WorkTaskInstance) workItem.getTaskInstance())
	 ...
	 */
	public void completeWorkItemAndJumpTo(IWorkItemInstance workItem,
		...
		boolean isInSameLine = workflowProcess.isInSameLine(thisActivityId,
				targetActivityId);
		WorkTaskInstance thisTaskInst = (WorkTaskInstance) workItem.getTaskInstance();
		if (!isInSameLine) {
			throw new EngineException(
					thisTaskInst.getProcessInstanceId(),
					thisTaskInst.getWorkflowProcess(),
	 ...
	 */
	public void completeWorkItemAndJumpToEx(IWorkItemInstance workItem,
											String targetActivityId, String comments) throws EngineException,
			KernelException {
...
//		}
		...
		List<ITokenInstance> allTokens = persistenceService.findTokensForProcessInstance(thisTaskInst.getProcessInstanceId(), null);
		...
		for (int i=0;i<allSynchronizersAndEnds.size();i++){
			...
			if (reachable){
				...
				ITokenInstance virtualToken = getJoinInfo(allTokens,synchronizer.getId()); //获取一个虚拟的综合性token
	 ...
	 * @return
	 */
	private ITokenInstance getJoinInfo(List<ITokenInstance> allTokens, String synchronizerId){
		boolean findTokens = false;
		Map<String, ITokenInstance> tokensMap = new HashMap<String, ITokenInstance>();
        for (int i = 0; i < allTokens.size(); i++) {
            ITokenInstance tmpToken = (ITokenInstance) allTokens.get(i);
            ...
            if (!tokensMap.containsKey(tmpFromActivityId)) {
                ...
                ITokenInstance tmpToken2 = (ITokenInstance) tokensMap.get(tmpFromActivityId);
        ...
        ITokenInstance virtualToken = new TokenInstance();
        int stepNumber = 0;
        List<ITokenInstance> tokensList = new ArrayList<ITokenInstance>(tokensMap.values());

        for (int i = 0; i < tokensList.size(); i++) {
            ITokenInstance _token = (ITokenInstance) tokensList.get(i);
            ...
            if (_token.isAlive()) {
                ...
                if (oldFromActivityId == null || oldFromActivityId.trim().equals("")) {
                	...
                	virtualToken.setFromActivityId(oldFromActivityId + ITokenInstance.FROM_ACTIVITY_ID_SEPARATOR + _token.getFromActivityId());
                }
            }
	 ...
	 */
	public void rejectWorkItem(IWorkItemInstance workItem, String comments)
			throws EngineException, KernelException {
		Activity thisActivity = workItem.getTaskInstance().getActivity();
		WorkTaskInstance thisTaskInstance = (WorkTaskInstance) workItem
				.getTaskInstance();
        ...
        //处理拒收的边界问题
        if(thisTaskInstance.getFromActivityId().equals(ITokenInstance.FROM_START_NODE)){
                throw new EngineException(
                                thisTaskInstance.getProcessInstanceId(),
                                thisTaskInstance.getWorkflowProcess(),
                                thisTaskInstance.getTaskId(),
                                "Reject operation refused!Because the from activityId equals "+ ITokenInstance.FROM_START_NODE );
        }
        //----added by wmj2003 20090915 ---end---
		IPersistenceService persistenceService = this.rtCtx
				.getPersistenceService();
		List<IWorkTaskInstance> siblingTaskInstancesList = null;
		...
		StringTokenizer tokenizer = new StringTokenizer(thisTaskInstance
				.getFromActivityId(), ITokenInstance.FROM_ACTIVITY_ID_SEPARATOR);
		...
		try {
			...
			((WorkItemInstance) workItem).setState(IWorkItemInstance.CANCELED);
			((WorkItemInstance) workItem).setEndTime(rtCtx.getCalendarService()
					.getSysDate());
			rtCtx.getPersistenceService().saveOrUpdateWorkItemInstance(workItem);
			...
			for (int i = 0; i < fromActivityIdList.size(); i++) {
				...
				TokenInstance newToken = new TokenInstance();
				((TokenInstance) newToken).setAlive(true);
				((TokenInstance) newToken).setNodeId(fromActivityId);
				newToken.setProcessInstanceId(thisTaskInstance
						.getProcessInstanceId());
				newToken.setProcessInstance(((WorkTaskInstance) thisTaskInstance)
						.getAliveProcessInstance());
				newToken.setFromActivityId(thisTaskInstance.getActivityId());
				newToken.setStepNumber(newStepNumber);
				newToken.setValue(0);
				persistenceService.saveOrUpdateTokenInstance(newToken);
			...
			if (synchronizerInstance.getEnteringTransitionInstances().size() > fromActivityIdList
					.size()) {
				TokenInstance supplementToken = new TokenInstance();
				((TokenInstance) supplementToken).setAlive(false);
				((TokenInstance) supplementToken).setNodeId(synchronizerInstance
						.getSynchronizer().getId());
				supplementToken.setProcessInstanceId(thisTaskInstance
						.getProcessInstanceId());
				supplementToken
						.setProcessInstance(((WorkTaskInstance) thisTaskInstance)
								.getAliveProcessInstance());
				...
				persistenceService.saveOrUpdateTokenInstance(supplementToken);
			}
		} finally {
	 ...
	 */
	public IWorkItemInstance withdrawWorkItem(IWorkItemInstance workItem)
			throws EngineException, KernelException {
		Activity thisActivity = workItem.getTaskInstance().getActivity();
		WorkTaskInstance thisTaskInstance = (WorkTaskInstance) workItem
				.getTaskInstance();
		...
		IPersistenceService persistenceService = this.rtCtx
				.getPersistenceService();
		List<IWorkTaskInstance> targetTaskInstancesList = null;
		...
		if (targetTaskInstancesList == null
			...
			WorkTaskInstance taskInstance = (WorkTaskInstance) targetTaskInstancesList
					.get(0);
		...
		for (int i = 0; targetTaskInstancesList != null
			...
			WorkTaskInstance targetTaskInstanceTmp = (WorkTaskInstance) targetTaskInstancesList
					.get(i);
		...
		try {
			...
			for (int i = 0; i < targetTaskInstancesList.size(); i++) {
				WorkTaskInstance taskInstTemp = (WorkTaskInstance) targetTaskInstancesList
						.get(i);
				...
				if (!targetActivityIdList
					...
					if (theFromActivityIds.length() == 0) {
						...
						theFromActivityIds.append(
								ITokenInstance.FROM_ACTIVITY_ID_SEPARATOR).append(
			...
			TokenInstance newToken = new TokenInstance();
			((TokenInstance) newToken).setAlive(true);
			((TokenInstance) newToken).setNodeId(workItem.getTaskInstance()
					.getActivityId());
			newToken.setProcessInstanceId(thisTaskInstance
					.getProcessInstanceId());
			newToken.setProcessInstance(((WorkTaskInstance) thisTaskInstance)
			...
			List<IWorkItemInstance>  workItems = persistenceService.findTodoWorkItems(workItem
			...
			return (IWorkItemInstance) workItems.get(0);
		} finally {
			session.setWithdrawOrRejectOperationFlag(false);
		}
	}
	 ...
	 */
	public IWorkItemInstance reassignWorkItemTo(IWorkItemInstance workItem, String actorId,
												String comments) {
		WorkItemInstance newWorkItem = new WorkItemInstance();
		...
		rtCtx.getPersistenceService().saveOrUpdateWorkItemInstance(newWorkItem);
