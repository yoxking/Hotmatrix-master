...
public class ReportViewController extends BaseViewController {
                        ...
                        @RequestParam(defaultValue = "10",value = "pageSize") Integer pageSize)
    {
        ...
        List<CctReportinfo> infoList=reportinfoService.getRecordsByPaging(loginUser.getAppCode(),pageIndex,pageSize,condition,"id","desc");
        if(infoList!=null&&infoList.size()>0){
            for(CctReportinfo info:infoList){
    ...
    public String detail(ModelMap model,@RequestParam("id") String id) {
        ...
        CctReportinfo info = reportinfoService.getRecordByNo(loginUser.getAppCode(), id);
