...
import com.benet.system.service.ISysUserService;
...
public class SysLoginService
{
    ...
    public SysUser login(String username, String password)
    {
        ...
        {
            AsyncManager.me().execute(AsyncFactory.recordLogininfor(username, PubConstants.LOGIN_FAIL, MessageUtils.message("not.null")));
        ...
        if (user == null)
        {
            AsyncManager.me().execute(AsyncFactory.recordLogininfor(username, PubConstants.LOGIN_FAIL, MessageUtils.message("user.not.exists")));
            throw new UserNotExistsException();
        }
        
        if (UserStatus.DELETED.getCode().equals(user.getDelFlag()))
        {
            AsyncManager.me().execute(AsyncFactory.recordLogininfor(username, PubConstants.LOGIN_FAIL, MessageUtils.message("user.password.delete")));
            throw new UserDeleteException();
        }
        
        if (UserStatus.DISABLE.getCode().equals(user.getStatus()))
        {
            AsyncManager.me().execute(AsyncFactory.recordLogininfor(username, PubConstants.LOGIN_FAIL, MessageUtils.message("user.blocked", user.getRemark())));
